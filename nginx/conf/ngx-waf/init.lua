...
closeWebDav=optionIsOn(closeWebDav)
compressJS=optionIsOn(compressJS)
compressCSS=optionIsOn(compressCSS)
...
function closewebdav()
    if closeWebDav then
       local md=ngx.req.get_method()
       if ngxmatch(md,"GET|POST|HEAD","imjo") == nil then
           ngx.exit(403)
           return true
       end
    end
...
function compresscss(css)
    if compressCSS then
        return sendbody('compresscss',css)
    end
    return css
end

function sendbody(v,o) 
    local tab = {}
    tab['option'] = v
    tab['body'] = o
    ...
    '/nodejs',
    { method = ngx.HTTP_POST, body = jsonData } )
